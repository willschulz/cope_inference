#!/bin/bash
# Start the CoPE-A-9B vLLM inference service (high throughput)
...
if [ -f cope.pid ]; then
    ...
    if kill -0 "$PID" 2>/dev/null; then
        ...
        echo "Run ./stop.sh first to stop the existing service."
        exit 1
    else
        rm -f cope.pid
    fi
...
echo "Starting CoPE-A-9B vLLM inference service..."
echo "Logs will be written to: $SCRIPT_DIR/cope.log"

# Start the vLLM service in the background
nohup python cope_vllm_service.py > cope.log 2>&1 &
PID=$!
echo $PID > cope.pid
